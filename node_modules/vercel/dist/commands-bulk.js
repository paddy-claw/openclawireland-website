...
import "./chunks/chunk-IGWOW3XS.js";
import {
...
async function activity(client) {
  ...
  switch (subcommand) {
    ...
    default: {
      ...
      const listFn = (await import("./chunks/list-NWJQIUJG.js")).default;
...
async function alerts(client) {
  ...
  telemetry2.trackCliSubcommandLs(subcommandOriginal);
  const listFn = (await import("./chunks/list-5FH7EZG5.js")).default;
  return listFn(client, telemetry2);
}
...
function formatVariantForDisplay(variant) {
  ...
  if (variant.label) {
    parts.push(import_chalk50.default.dim(variant.label));
  }
  return parts.join(" ");
}
...
function formatAvailableVariant(variant) {
  ...
  return `${value} ${import_chalk50.default.dim(variant.label)}`;
}
function formatStyledVariantValue(value) {
  const formattedValue = formatVariantValue(value);
  if (typeof value !== "string") {
    return import_chalk50.default.bold(formattedValue);
  }
  return `"${import_chalk50.default.bold(formattedValue.slice(1, -1))}"`;
}
...
function printFlagDetails({
  ...
  output_manager_default.log(
    `
Feature flag ${import_chalk51.default.bold(flag.slug)} for ${projectSlugLink}
`
  );
  output_manager_default.print(`  ${import_chalk51.default.cyan(dashboardUrl)}

`);
  output_manager_default.print(`  ${import_chalk51.default.dim("ID:")}           ${flag.id}
`);
  output_manager_default.print(`  ${import_chalk51.default.dim("Kind:")}         ${flag.kind}
`);
  output_manager_default.print(
    `  ${import_chalk51.default.dim("State:")}        ${flag.state === "active" ? import_chalk51.default.green(flag.state) : import_chalk51.default.gray(flag.state)}
`
  );
  if (flag.description) {
    output_manager_default.print(`  ${import_chalk51.default.dim("Description:")}  ${flag.description}
`);
  }
  if (showTimestamps) {
    output_manager_default.print(
      `  ${import_chalk51.default.dim("Created:")}      ${formatDate(flag.createdAt)}
`
    );
    output_manager_default.print(
      `  ${import_chalk51.default.dim("Updated:")}      ${formatDate(flag.updatedAt)}
`
    );
  }
  output_manager_default.print(`
  ${import_chalk51.default.dim("Variants:")}
`);
  for (const [index, variant] of flag.variants.entries()) {
    output_manager_default.print(`    ${formatVariantListSummary(variant)}
`);
    output_manager_default.print(`      ${import_chalk51.default.dim(`id: ${variant.id}`)}
`);
...
function printFlagEnvironmentDetails(flag, settings, environments) {
  ...
  output_manager_default.print(`
  ${import_chalk51.default.dim("Environments:")}
`);
  for (const [envName, envConfig] of sortedEnvs) {
    if (envConfig.reuse?.active) {
      output_manager_default.print(
        `    ${import_chalk51.default.bold(envName)}: reuses ${import_chalk51.default.cyan(envConfig.reuse.environment)} environment
`
      );
      continue;
    }
    if (envConfig.active) {
      ...
      output_manager_default.print(`    ${import_chalk51.default.bold(envName)}: ${envSummary}
`);
      if (envConfig.targets && Object.keys(envConfig.targets).length > 0) {
        output_manager_default.print(`      ${import_chalk51.default.dim("Targeting:")}
`);
          ...
          for (const [entityKind, attributes] of Object.entries(entityKinds)) {
            for (const [attribute, values] of Object.entries(attributes)) {
              const valueList = values.map((v) => {
                ...
                return label ? `${v.value} ${import_chalk51.default.gray(label)}` : v.value;
              }).join(", ");
              output_manager_default.print(
                `        ${import_chalk51.default.dim(`${entityKind}.${attribute}:`)} ${valueList} ${import_chalk51.default.dim("\u2192")} ${variantSummary}
`
              );
            }
          }
      ...
      if (envConfig.rules && envConfig.rules.length > 0) {
        output_manager_default.print(`      ${import_chalk51.default.dim("Rules:")}
`);
        for (const rule of envConfig.rules) {
          const outcome = formatEnvironmentOutcome(rule.outcome, flag.variants);
          output_manager_default.print(`        ${import_chalk51.default.dim("\u2192")} ${outcome}
`);
          for (const condition of rule.conditions) {
            const { text, listItems } = formatCondition2(condition, settings);
            output_manager_default.print(`          ${import_chalk51.default.dim("if")} ${text}
`);
            if (listItems && listItems.length > 0) {
              for (const item of listItems) {
                output_manager_default.print(`             ${import_chalk51.default.dim("-")} ${item}
`);
              }
            }
          }
      ...
      if (hasCustomConfiguration && envConfig.fallthrough) {
        ...
        if (fallthrough.type === "variant") {
          ...
          output_manager_default.print(`      ${import_chalk51.default.dim("Default:")} ${defaultSummary}
`);
          ...
          output_manager_default.print(`      ${import_chalk51.default.dim("Default split:")} ${weights}
`);
        }
      }
    } else {
      ...
      output_manager_default.print(`    ${import_chalk51.default.bold(envName)}: ${pausedSummary}
`);
    }
  }
  output_manager_default.print("\n");
}
function getSortedEnvironmentEntries(flag, environments) {
  const envOrder = ["production", "preview", "development"];
  const selectedEnvironments = environments ? new Set(environments) : void 0;
  return Object.entries(flag.environments).filter(
    ([envName]) => selectedEnvironments ? selectedEnvironments.has(envName) : true
  ).sort(([a], [b]) => {
...
function hasCustomConfigurationEnabled(envConfig) {
  return Boolean(envConfig.targets && Object.keys(envConfig.targets).length > 0) || envConfig.rules.length > 0 || envConfig.fallthrough.type === "split";
}
function formatEnvironmentOutcome(outcome, variants) {
...
function formatEnvironmentVariantSummary(variant, fallback) {
  if (!variant) {
    return import_chalk51.default.bold(fallback);
  }
  if (variant.label) {
    return import_chalk51.default.bold(variant.label);
  }
  return import_chalk51.default.bold(formatVariantValue(variant.value));
}
function formatVariantListSummary(variant) {
  ...
  return `${value}: ${import_chalk51.default.gray(variant.label)}`;
}
function formatCondition2(condition, settings) {
  ...
  const cmp = import_chalk51.default.dim(formatComparison(condition.cmp));
    ...
    if (condition.rhs.type === "list" && Array.isArray(condition.rhs.items)) {
      const items = condition.rhs.items.map((item) => {
        ...
        if (condition.lhs.type === "entity") {
          ...
          return label ? `${itemValue} ${import_chalk51.default.gray(label)}` : itemValue;
        }
        return itemValue;
      });
  ...
  if (condition.lhs.type === "entity") {
    ...
    rhs = label ? `${condition.rhs} ${import_chalk51.default.gray(label)}` : String(condition.rhs);
  } else {
...
var FlagsCreateTelemetryClient = class extends TelemetryClient {
...
async function create(client, argv) {
  ...
  if (!slug) {
    ...
    output_manager_default.log(`Example: ${getCommandName("flags create my-feature")}`);
    return 1;
  }
  ...
  try {
    ...
    output_manager_default.stopSpinner();
    output_manager_default.success(
      `Feature flag ${import_chalk52.default.bold(flag.slug)} created successfully`
    );
    printFlagDetails({
      flag,
      projectSlugLink,
...
async function getVariants(client, kind, variantInputs) {
  if (kind === "boolean") {
...
async function collectVariantsInteractively(client, kind) {
  const variants = [];
  let addAnother = true;
...
function createEnvironmentConfig(pausedVariantId, fallthroughVariantId, active) {
  return {
    revision: 0,
    active,
    pausedOutcome: {
...
function formatVariantInput(valueInput, label) {
...
// src/commands/flags/open.ts
async function openFlag(client, argv) {
  ...
  try {
    ...
    if (client.stdout.isTTY) {
      ...
      void (0, import_open2.default)(url).catch(() => void 0);
    } else {
...
async function resolveOptionalInput(client, input, defaultValue, promptMessage) {
...
var FlagsUpdateTelemetryClient = class extends TelemetryClient {
  trackCliArgumentFlag(flag) {
    if (flag) {
      this.trackCliArgument({
        arg: "flag",
...
// src/commands/flags/update.ts
var DEFAULT_UPDATE_MESSAGE = "Updated via CLI";
async function update(client, argv) {
  ...
  try {
    ...
    if (flag.state === "archived") {
      output_manager_default.error(
        `Flag ${import_chalk53.default.bold(flag.slug)} is archived and cannot be updated`
      );
      return 1;
    }
    ...
    if (changedVariants.length === 0) {
      output_manager_default.warn(`Flag ${import_chalk53.default.bold(flag.slug)} is already up to date`);
      return 0;
    }
    ...
    output_manager_default.stopSpinner();
    output_manager_default.success(`Feature flag ${import_chalk53.default.bold(flag.slug)} has been updated`);
    for (const variant of changedVariants) {
      output_manager_default.log(
        `  ${import_chalk53.default.dim("Variant:")} ${formatVariantForDisplay(variant)}`
      );
    }
  } catch (err) {
    output_manager_default.stopSpinner();
...
async function collectVariantUpdate(client, flag, input) {
  ...
  if (flag.kind !== "boolean" && nextValueInput === void 0 && client.stdin.isTTY) {
  ...
  if (nextLabel === void 0 && client.stdin.isTTY) {
...
function ensureVariantUpdateInput(valueInput, label) {
  if (!valueInput && !label) {
    throw new Error(
...
async function resolveSelectedVariant(client, flag, selector) {
  if (selector) {
  ...
  const selectedVariantId = await client.input.select({
    ...
    choices: flag.variants.map((variant) => ({
      name: `${formatVariantForDisplay(variant)} ${import_chalk53.default.dim(`[id: ${variant.id}]`)}`,
      value: variant.id
    }))
  });
...
async function promptForVariantValueUpdate(client, flag, selectedVariant) {
...
async function promptForVariantLabelUpdate(client, selectedVariant) {
  const currentLabel = selectedVariant.label;
  const labelResponse = await client.input.text({
    message: currentLabel ? `Enter a new label for ${formatVariantForDisplay(selectedVariant)} (press Enter to keep ${JSON.stringify(currentLabel)}):` : `Enter an optional label for ${formatVariantForDisplay(selectedVariant)} (press Enter to skip):`
  });
...
function applyVariantUpdates(flag, updates) {
  ...
  for (const update2 of updates) {
    const result = resolveVariant(update2.selector, variants);
    if (result.error || !result.variant) {
      throw new Error(result.error || `Variant "${update2.selector}" not found`);
    }
    const nextValue = update2.valueInput !== void 0 ? parseUpdatedVariantValue(update2.valueInput, flag.kind, result.variant) : result.variant.value;
    const nextLabel = update2.label ?? result.variant.label;
  ...
  return {
    variants,
    changedVariants: variants.filter(
      (variant) => changedVariantIds.has(variant.id)
    )
  };
}
...
async function resolveFlagEnvironment(client, flag, environment, promptMessage, options = {}) {
  ...
  if (!nextEnvironment) {
    ...
    const availableEnvironments = STANDARD_ENVIRONMENTS.filter(
      (env) => Object.prototype.hasOwnProperty.call(flag.environments, env)
    );
    if (availableEnvironments.length === 0) {
      throw new Error("No valid environments found for this flag");
    }
    if (options.showEnvironmentDetails) {
      printFlagEnvironmentDetails(flag, void 0, availableEnvironments);
    }
    nextEnvironment = await client.input.select({
      message: promptMessage,
      choices: availableEnvironments.map((env) => {
        return {
          name: options.decorateChoices === false ? env : formatEnvironmentChoiceLabel(env, flag.environments[env]),
          value: env
        };
      })
    });
  }
  if (!STANDARD_ENVIRONMENTS.includes(nextEnvironment)) {
    throw new Error(
...
function formatEnvironmentChoiceLabel(envName, envConfig) {
  const status3 = envConfig?.active ? import_chalk54.default.green("active") : import_chalk54.default.yellow("paused");
...
function getBooleanVariant(flag, value) {
  const variant = flag.variants.find((candidate) => candidate.value === value);
  if (!variant) {
    throw new Error(
      `Flag ${import_chalk54.default.bold(flag.slug)} is missing the standard boolean variants`
    );
  }
  return variant;
}
async function resolveFlagUpdateMessage(client, message, defaultMessage) {
  if (message !== void 0) {
    return message;
  }
  if (!client.stdin.isTTY) {
...
// src/util/telemetry/commands/flags/set.ts
var FlagsSetTelemetryClient = class extends TelemetryClient {
  trackCliArgumentFlag(flag) {
    if (flag) {
      this.trackCliArgument({
        arg: "flag",
...
// src/commands/flags/set.ts
async function set2(client, argv) {
  ...
  let parsedArgs;
  const flagsSpecification = getFlagsSpecification(setSubcommand2.options);
  try {
    parsedArgs = parseArguments(argv, flagsSpecification);
  } catch (err) {
    printError(err);
  ...
  try {
    ...
    if (flag.state === "archived") {
      output_manager_default.error(
        `Flag ${import_chalk55.default.bold(flag.slug)} is archived and cannot be set`
      );
      return 1;
    }
    ...
    if (isOverridingEnvironmentToVariant(envConfig, selectedVariant.id)) {
      output_manager_default.warn(
        `Flag ${import_chalk55.default.bold(flag.slug)} is already serving ${formatVariantForDisplay(selectedVariant)} in ${selectedEnvironment}`
      );
      return 0;
    }
    ...
    output_manager_default.spinner(presentation.spinner(selectedEnvironment));
    ...
    output_manager_default.log(
      `  ${import_chalk55.default.dim("Serving variant:")} ${formatVariantForDisplay(selectedVariant)}`
    );
  } catch (err) {
    output_manager_default.stopSpinner();
    printError(err);
...
async function resolveVariantToSet(client, flag, selector) {
  if (selector) {
    const result = resolveVariant(selector, flag.variants);
    if (result.error || !result.variant) {
      throw new Error(result.error || `Variant "${selector}" not found`);
...
function getSetPresentation(flag, variant) {
  if (flag.kind === "boolean" && variant.value === true) {
    return {
      spinner: (environment) => `Enabling flag in ${environment}...`,
      success: (slug, environment) => `Feature flag ${import_chalk55.default.bold(slug)} has been enabled in ${import_chalk55.default.bold(environment)}`,
  ...
  if (flag.kind === "boolean" && variant.value === false) {
    return {
      ...
      success: (slug, environment) => `Feature flag ${import_chalk55.default.bold(slug)} has been disabled in ${import_chalk55.default.bold(environment)}`,
      defaultMessage: (environment) => `Disabled for ${environment} via CLI`
    };
  }
  return {
    spinner: (environment) => `Setting variant in ${environment}...`,
    success: (slug, environment) => `Feature flag ${import_chalk55.default.bold(slug)} has been set in ${import_chalk55.default.bold(environment)}`,
...
async function rm5(client, argv) {
  ...
  try {
    ...
    if (flag.state !== "archived") {
      output_manager_default.error(
        `Flag ${import_chalk56.default.bold(flag.slug)} must be archived before it can be deleted. Run ${getCommandName(`flags archive ${flag.slug}`)} first.`
      );
      return 1;
    }
    if (!skipConfirmation) {
      ...
      const confirmed = await client.input.confirm(
        `Are you sure you want to delete ${import_chalk56.default.bold(flag.slug)}? This action cannot be undone.`,
        false
      );
      if (!confirmed) {
    ...
    output_manager_default.stopSpinner();
    output_manager_default.success(`Feature flag ${import_chalk56.default.bold(flag.slug)} has been deleted`);
  } catch (err) {
    output_manager_default.stopSpinner();
    printError(err);
    return 1;
...
async function archive(client, argv) {
  ...
  try {
    ...
    output_manager_default.stopSpinner();
    if (flag.state === "archived") {
      output_manager_default.warn(`Flag ${import_chalk57.default.bold(flag.slug)} is already archived`);
      return 0;
    }
    if (!skipConfirmation) {
      if (!client.stdin.isTTY) {
      ...
      const confirmed = await client.input.confirm(
        `Are you sure you want to archive ${import_chalk57.default.bold(flag.slug)}?`,
        false
      );
      if (!confirmed) {
        output_manager_default.log("Aborted");
    ...
    output_manager_default.stopSpinner();
    output_manager_default.success(`Feature flag ${import_chalk57.default.bold(flag.slug)} has been archived`);
    output_manager_default.log(
      `
To restore this flag, visit the dashboard: ${import_chalk57.default.cyan(getFlagsDashboardUrl(link.org.slug, project.name) + "/archive")}`
    );
  } catch (err) {
    output_manager_default.stopSpinner();
    printError(err);
...
function logNonBooleanFlagGuidance(flag, {
  ...
  const suggestedEnvironment = environment && environment in flag.environments ? environment : void 0;
  output_manager_default.warn(
    `The ${getCommandName(`flags ${attemptedSubcommand}`)} command only works with boolean flags.`
  );
  output_manager_default.log(
    `Flag ${import_chalk58.default.bold(flag.slug)} is a ${import_chalk58.default.cyan(flag.kind)} flag. Set a specific variant instead:`
  );
  output_manager_default.log(
    `  ${getCommandName(
  ...
  if (flag.variants.length > 0) {
    output_manager_default.log("Available variants:");
    for (const variant of flag.variants) {
      output_manager_default.log(`  - ${formatVariantForDisplay(variant)}`);
    }
  }
  output_manager_default.log(
    `See full flag details with ${getCommandName(`flags inspect ${flag.slug}`)}`
  );
  output_manager_default.log(`Open in the dashboard: ${import_chalk58.default.cyan(dashboardUrl)}`);
}
...
async function disable(client, argv) {
  ...
  try {
    ...
    if (flag.state === "archived") {
      output_manager_default.error(
        `Flag ${import_chalk59.default.bold(flag.slug)} is archived and cannot be disabled`
      );
      return 1;
    }
    ...
    if (isPausingEnvironmentToVariant(envConfig, selectedVariant.id)) {
      output_manager_default.warn(
        `Flag ${import_chalk59.default.bold(flag.slug)} is already disabled in ${environment}`
      );
      return 0;
    }
    const updateMessage = await resolveFlagUpdateMessage(
    ...
    output_manager_default.stopSpinner();
    output_manager_default.success(
      `Feature flag ${import_chalk59.default.bold(flag.slug)} has been disabled in ${import_chalk59.default.bold(environment)}`
    );
    output_manager_default.log(
      `  ${import_chalk59.default.dim("Serving variant:")} ${formatVariantForDisplay(selectedVariant)}`
    );
  } catch (err) {
    output_manager_default.stopSpinner();
    printError(err);
...
function getDefaultDisableMessage(environment) {
...
async function enable(client, argv) {
  ...
  try {
    ...
    if (flag.state === "archived") {
      output_manager_default.error(
        `Flag ${import_chalk60.default.bold(flag.slug)} is archived and cannot be enabled`
      );
      return 1;
    }
    if (flag.kind !== "boolean") {
    ...
    if (isPausingEnvironmentToVariant(envConfig, onVariant.id)) {
      output_manager_default.warn(
        `Flag ${import_chalk60.default.bold(flag.slug)} is already enabled in ${environment}`
      );
      return 0;
    }
    const updateMessage = await resolveFlagUpdateMessage(
    ...
    output_manager_default.stopSpinner();
    output_manager_default.success(
      `Feature flag ${import_chalk60.default.bold(flag.slug)} has been enabled in ${import_chalk60.default.bold(environment)}`
    );
    output_manager_default.log(
      `  ${import_chalk60.default.dim("Serving variant:")} ${formatVariantForDisplay(onVariant)}`
    );
  } catch (err) {
    output_manager_default.stopSpinner();
    printError(err);
...
function getDefaultEnableMessage(environment) {
...
Create one with: ${getCommandName("flags sdk-keys add --type server --environment production")}`
      ...
      output_manager_default.log(
        `${import_chalk61.default.bold(keys.length)} SDK key${keys.length === 1 ? "" : "s"} found for ${projectSlugLink}`
      );
...
function printSdkKeysTable(keys) {
  ...
  const rows = keys.map((key) => [
    import_chalk61.default.dim(key.hashKey.slice(0, 12) + "..."),
    getTypeLabel(key.type),
    key.environment,
    key.label || import_chalk61.default.dim("-"),
...
function getTypeLabel(type) {
  switch (type) {
    case "server":
      return import_chalk61.default.blue("server");
    case "client":
      return import_chalk61.default.green("client");
    case "mobile":
      return import_chalk61.default.yellow("mobile");
    default:
      return type;
  }
}
...
async function sdkKeysAdd(client, argv) {
  ...
  try {
    ...
    output_manager_default.print("\n");
    output_manager_default.print(`  ${import_chalk62.default.dim("Hash Key:")}     ${key.hashKey}
`);
    output_manager_default.print(`  ${import_chalk62.default.dim("Type:")}         ${key.type}
`);
    output_manager_default.print(`  ${import_chalk62.default.dim("Environment:")}  ${key.environment}
`);
    if (key.label) {
      output_manager_default.print(`  ${import_chalk62.default.dim("Label:")}        ${key.label}
`);
    }
    if (key.keyValue) {
      output_manager_default.print("\n");
      ...
      output_manager_default.print(`
  ${import_chalk62.default.cyan(key.keyValue)}
`);
    }
    if (key.connectionString) {
      output_manager_default.print("\n");
      output_manager_default.log(`${import_chalk62.default.dim("Connection string:")}`);
      output_manager_default.print(`  ${import_chalk62.default.cyan(key.connectionString)}
`);
    }
  } catch (err) {
    output_manager_default.stopSpinner();
...
async function sdkKeysRm(client, argv) {
  ...
  try {
    ...
    output_manager_default.stopSpinner();
    output_manager_default.success(
      `SDK key ${import_chalk63.default.bold(hashKey.slice(0, 12) + "...")} has been deleted`
    );
  } catch (err) {
    output_manager_default.stopSpinner();
    printError(err);
...
var COMMAND_CONFIG13 = {
  ...
  set: getCommandAliases(setSubcommand2),
...
async function main8(client) {
  ...
  switch (subcommand) {
    ...
    case "set":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("flags", subcommandOriginal);
        printHelp(setSubcommand2);
        return 2;
      }
      telemetry2.trackCliSubcommandSet(subcommandOriginal);
      return set2(client, args);
    case "rm":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("flags", subcommandOriginal);
...
async function connect(client, argv) {
  ...
  if (!gitConfig) {
    output_manager_default.error(
      `No local Git repository found. Run ${import_chalk64.default.cyan(
        "`git clone <url>`"
  ...
  if (!remoteUrls) {
    output_manager_default.error(
      `No remote URLs found in your Git config. Make sure you've configured a remote repo in your local Git config. Run ${import_chalk64.default.cyan(
        "`git remote --help`"
      )} for more details.`
    );
    return 1;
...
async function promptConnectArg({
  ...
  if (Object.keys(remoteUrls).length > 1) {
    ...
    output_manager_default.log(
      `Found a repository in your local Git Config: ${import_chalk64.default.cyan(
...
`
    ...
    const confirmDisconnect = autoConfirm || await client.input.confirm(
      `Are you sure you want to disconnect ${import_chalk65.default.cyan(
        `${linkOrg}/${repo}`
      )} from your project?`,
      false
    );
    if (confirmDisconnect) {
      ...
      output_manager_default.log(`Disconnected ${import_chalk65.default.cyan(`${linkOrg}/${repo}`)}.`);
    } else {
      output_manager_default.log("Canceled");
    }
  } else {
...
async function status(client) {
  ...
  const status3 = enabled ? import_chalk66.default.green("Enabled") : import_chalk66.default.red("Disabled");
  output_manager_default.print("\n");
  output_manager_default.log(`${import_chalk66.default.bold("Guidance status")}: ${status3}`);
  output_manager_default.print("\n");
  return 0;
}
...
async function extractExample(client, name, dir, force, ver = "v2") {
  ...
  return client.fetch(url, { json: false }).then(async (res) => {
    ...
    const successLog = `Initialized "${import_chalk67.default.bold(
      name
    )}" example in ${import_chalk67.default.bold(humanizePath(folder))}.`;
...
function prepareFolder(cwd, folder, force) {
  ...
  if (fs.existsSync(dest)) {
    if (!fs.lstatSync(dest).isDirectory()) {
      throw new Error(
        `Destination path "${import_chalk67.default.bold(
          folder
    ...
    try {
      ...
      throw new Error(`Could not create directory "${import_chalk67.default.bold(folder)}".`);
    }
  }
  return dest;
}
async function guess(client, exampleList, name) {
  const GuessError = new Error(
    `No example found for ${import_chalk67.default.bold(name)}, run ${getCommandName(
      "init"
  ...
  if (typeof found === "string") {
    if (await client.input.confirm(`Did you mean ${import_chalk67.default.bold(found)}?`, false)) {
      return found;
    }
  } else {
    throw GuessError;
...
var styleBuild = (build, times, longestSource) => {
  ...
  let pathColor = import_chalk68.default.cyan;
  if (isFailed(build)) {
    pathColor = import_chalk68.default.red;
  }
  const entry = entrypoint.padEnd(longestSource + padding);
  const prefix = hasOutput(build) ? "\u250C" : "\u2576";
  return `${import_chalk68.default.grey(prefix)} ${pathColor(entry)}${time}`;
};
var styleHiddenBuilds = (commonPath, buildGroup, times, longestSource, isHidden2 = false) => {
  ...
  if (isHidden2) {
    pathColor = import_chalk68.default.grey;
  }
  return `${import_chalk68.default.grey(prefix)} ${pathColor(entry)}${time}`;
};
var styleOutput = (output, readyState, isLast) => {
  ...
  const finalSize = size ? ` ${import_chalk68.default.grey(`(${(0, import_bytes3.default)(size)})`)}` : "";
  let color = import_chalk68.default.grey;
  let finalRegion = "";
  if (isReady({ readyState })) {
    color = import_chalk68.default;
  } else if (isFailed({ readyState })) {
    color = import_chalk68.default.red;
  }
  if (lambda) {
    const { deployedTo } = lambda;
    if (deployedTo && deployedTo.length > 0) {
      finalRegion = ` ${import_chalk68.default.grey(`[${deployedTo.join(", ")}]`)}`;
    }
  }
  const corner = isLast ? "\u2514\u2500\u2500" : "\u251C\u2500\u2500";
  const main20 = prefix + path3 + finalSize + finalRegion;
  return `${import_chalk68.default.grey(corner)} ${color(main20)}`;
};
...
function routes(routes2) {
  ...
  const arrow = import_chalk69.default.grey("->");
  for (const item of routes2) {
    if ("handle" in item) {
      toPrint += `${import_chalk69.default.grey("\u2576")} ${import_chalk69.default.cyan(item.handle)}`;
      continue;
    }
    ...
    const suffix = last ? "" : `
`;
    const finalSrc = import_chalk69.default.cyan(src.padEnd(longestSrc + padding2));
    const finalDest = dest ? `${arrow}${space}${dest}` : `  ${space}${destSpace}`;
    const finalStatus = status3 ? import_chalk69.default.grey(`[${status3}]`) : "";
...
`;
      ...
      for (const header of headerKeys) {
        ...
        const suffix2 = last2 ? "" : `
`;
        const prefix2 = import_chalk69.default.grey(last2 ? "\u2514\u2500\u2500" : "\u251C\u2500\u2500");
        finalHeaders += `${prefix2} ${header}: ${value}${suffix2}`;
      }
    }
    const prefix = import_chalk69.default.grey(finalHeaders ? "\u250C" : "\u2576");
...
async function inspect3(client) {
  ...
  output_manager_default.spinner(
    `Fetching deployment "${deploymentIdOrHost}" in ${import_chalk70.default.bold(contextName)}`
  );
  ...
  if (asJson) {
    ...
    print(`${import_chalk70.default.cyan("status")}	${stateString(deployment.readyState)}
`);
  } else {
...
function stateString(s) {
  ...
  switch (s) {
    case "INITIALIZING":
    case "BUILDING":
      return import_chalk70.default.yellow(CIRCLE) + sTitle;
    case "ERROR":
      return import_chalk70.default.red(CIRCLE) + sTitle;
    case "READY":
      return import_chalk70.default.green(CIRCLE) + sTitle;
    case "QUEUED":
      return import_chalk70.default.gray(CIRCLE) + sTitle;
    case "CANCELED":
      return import_chalk70.default.gray(CIRCLE) + sTitle;
    default:
      return import_chalk70.default.gray("UNKNOWN");
  }
}
async function printDetails({
  ...
  output_manager_default.log(
    `Fetched deployment "${import_chalk70.default.bold(deployment.url)}" in ${import_chalk70.default.bold(
      contextName
  ...
  print("\n");
  print(import_chalk70.default.bold("  General\n\n"));
  print(`    ${import_chalk70.default.cyan("id")}		${id}
`);
  print(`    ${import_chalk70.default.cyan("name")}	${name}
`);
  ...
  print(`    ${import_chalk70.default.cyan("target")}	`);
  print(
...
`
  ...
  print(`    ${import_chalk70.default.cyan("status")}	${stateString(readyState)}
`);
  print(`    ${import_chalk70.default.cyan("url")}		https://${url}
`);
  if (createdAt) {
    print(
      `    ${import_chalk70.default.cyan("created")}	${new Date(createdAt)} ${elapsed(
        Date.now() - createdAt,
        true
      )}
`
  ...
  if (aliases !== void 0 && aliases.length > 0) {
    print(import_chalk70.default.bold("  Aliases\n\n"));
    let aliasList = "";
    for (const alias2 of aliases) {
      aliasList += `${import_chalk70.default.gray("\u2576")} https://${alias2}
`;
    }
...
async function postProvisionSetup(client, resourceName, resourceId, contextName, options = {}) {
  ...
  output_manager_default.spinner(
    `Connecting ${import_chalk71.default.bold(resourceName)} to ${import_chalk71.default.bold(project.name)}...`
  );
  ...
  output_manager_default.stopSpinner();
  output_manager_default.log(
    `${import_chalk71.default.bold(resourceName)} successfully connected to ${import_chalk71.default.bold(project.name)}`
  );
...
function formatMetadataSchemaHelp(schema, integrationName, productSlug) {
  ...
  lines.push(import_chalk72.default.bold(header));
  lines.push("");
  ...
  for (const [key, prop] of entries) {
    ...
    const requiredSuffix = isRequired && !isServerHandledRegion(prop) ? import_chalk72.default.red(" (required)") : "";
    const typeHint = prop.type === "boolean" ? import_chalk72.default.dim(" (true/false)") : prop.type === "array" ? import_chalk72.default.dim(" (comma-separated)") : "";
    lines.push(`    ${import_chalk72.default.cyan(key)}${requiredSuffix}${typeHint}`);
    if (prop.description) {
    ...
    if (example) {
      lines.push(`      Example: ${import_chalk72.default.dim(example)}`);
    }
    lines.push("");
  }
  return lines.join("\n");
...
async function acceptTermsViaBrowser(client, integration, teamId, teamSlug, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  (0, import_open5.default)(url.href).catch(
...
Example: vercel ${commandName} ${integrationSlug}/${integration.products[0].slug}`
  ...
  output_manager_default.log(
    `Installing ${import_chalk73.default.bold(product.name)} by ${import_chalk73.default.bold(integration.name)} under ${import_chalk73.default.bold(contextName)}`
  );
...
Re-run with --installation-id to select one, e.g.:
  ...
  output_manager_default.success(
    `${product.name} successfully provisioned: ${import_chalk73.default.bold(resourceName)}`
  );
  ...
  output_manager_default.log(
    indent_default(
      `Guide: Run ${import_chalk73.default.cyan(`\`${guideCommand}\``)} for getting started guides and code snippets`,
      4
    )
  );
...
async function add5(client, args, flags, commandName) {
  ...
  output_manager_default.log(
    `Installing ${import_chalk74.default.bold(product.name)} by ${import_chalk74.default.bold(integration.name)} under ${import_chalk74.default.bold(contextName)}`
  );
...
async function confirmProductSelection(client, product, name, metadata, billingPlan) {
  ...
  output_manager_default.print(`${import_chalk74.default.dim(`- ${import_chalk74.default.bold("Name:")} ${name}`)}
`);
  for (const [key, value] of Object.entries(metadata)) {
    output_manager_default.print(
      `${import_chalk74.default.dim(`- ${import_chalk74.default.bold(`${product.metadataSchema.properties[key]["ui:label"]}:`)} ${value}`)}
`
    );
  }
  output_manager_default.print(
    `${import_chalk74.default.dim(`- ${import_chalk74.default.bold("Plan:")} ${billingPlan.name}`)}
`
  );
...
async function provisionStorageProduct(client, product, installation, name, metadata, billingPlan, authorizationId, integrationSlug, contextName, options = {}) {
  ...
  output_manager_default.success(
    `${product.name} successfully provisioned: ${import_chalk74.default.bold(name)}`
  );
...
function formatProductHelp(integrationSlug, products, commandName = "integration add") {
  ...
  lines.push("");
  lines.push(
    `  ${import_chalk75.default.dim("Available products for")} "${import_chalk75.default.bold(integrationSlug)}"${import_chalk75.default.dim(":")}`
  );
  lines.push("");
  const maxSlugLen = Math.max(...products.map((p) => p.slug.length));
  for (const product of products) {
    const paddedSlug = product.slug.padEnd(maxSlugLen);
    lines.push(`    ${import_chalk75.default.cyan(paddedSlug)}  ${product.name}`);
  }
  lines.push("");
  lines.push(`  ${import_chalk75.default.dim("Usage:")}`);
  lines.push("");
  lines.push(
    `    ${import_chalk75.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug}/<product-slug>`)}`
  );
  lines.push("");
  return lines.join("\n");
}
...
function formatBillingPlansHelp(productName, plans) {
  ...
  lines.push("");
  lines.push(
    `  ${import_chalk76.default.dim("Available billing plans for")} "${import_chalk76.default.bold(productName)}"${import_chalk76.default.dim(":")}`
  );
  lines.push("");
  const maxIdLen = Math.max(...enabledPlans.map((p) => p.id.length));
  for (const plan of enabledPlans) {
    const paddedId = plan.id.padEnd(maxIdLen);
    const cost = plan.cost ? import_chalk76.default.dim(` (${plan.cost})`) : "";
    lines.push(`    ${import_chalk76.default.cyan(paddedId)}  ${plan.name}${cost}`);
  }
  lines.push("");
  lines.push(`  ${import_chalk76.default.dim("Usage:")}`);
  lines.push("");
  lines.push(`    ${import_chalk76.default.cyan(`--plan ${enabledPlans[0].id}`)}`);
  lines.push("");
  return lines.join("\n");
}
...
function formatDynamicExamples(integrationSlug, products, commandName = "integration add") {
  const lines = [];
  lines.push("");
  lines.push(`  ${import_chalk77.default.dim("Examples:")}`);
  lines.push("");
  lines.push(`  ${import_chalk77.default.dim("-")} Install ${integrationSlug}`);
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug}`)}`
  );
  if (products.length > 1) {
    const firstProduct = products[0];
    lines.push("");
    lines.push(`  ${import_chalk77.default.dim("-")} Install a specific product`);
    lines.push("");
    lines.push(
      `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug}/${firstProduct.slug}`)}`
    );
  }
  lines.push("");
  lines.push(`  ${import_chalk77.default.dim("-")} Install with a custom resource name`);
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} --name my-resource`)}`
  );
  ...
  if (metadataExample) {
    lines.push("");
    lines.push(`  ${import_chalk77.default.dim("-")} Install with metadata`);
    lines.push("");
    lines.push(`    ${import_chalk77.default.cyan(`$ ${metadataExample}`)}`);
  }
  lines.push("");
  lines.push(`  ${import_chalk77.default.dim("-")} Install with a specific billing plan`);
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} --plan pro`)}`
  );
  lines.push("");
  lines.push(
    `  ${import_chalk77.default.dim("-")} Install and connect to specific environments only`
  );
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} -e production -e preview`)}`
  );
  lines.push("");
  lines.push(
    `  ${import_chalk77.default.dim("-")} Install without connecting to the current project`
  );
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} --no-connect`)}`
  );
  lines.push("");
  lines.push(
    `  ${import_chalk77.default.dim("-")} Install without pulling environment variables`
  );
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} --no-env-pull`)}`
  );
  lines.push("");
  lines.push(
    `  ${import_chalk77.default.dim("-")} Install with a prefix for environment variable names`
  );
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} --prefix NEON2_`)}`
  );
  lines.push("");
  lines.push(`  ${import_chalk77.default.dim("-")} Output as JSON`);
  lines.push("");
  lines.push(
    `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} --format=json`)}`
  );
  if (process.env.FF_AUTO_PROVISION_INSTALL !== "0") {
    lines.push("");
    lines.push(`  ${import_chalk77.default.dim("-")} Install using a specific installation`);
    lines.push("");
    lines.push(
      `    ${import_chalk77.default.cyan(`$ ${packageName} ${commandName} ${integrationSlug} --installation-id <id>`)}`
    );
  }
  lines.push("");
  return lines.join("\n");
...
function outputBalanceInformation(prepaymentInfo, resources, integrationSlug) {
  ...
  output_manager_default.log(
    `${import_chalk78.default.bold(`Balances and thresholds for ${integrationSlug}`)}:`
  );
...
`
  ...
  const headerMessage = project ? `Integration resources for project ${import_chalk79.default.bold(project.name)} in ${import_chalk79.default.bold(contextName)}:` : `Integrations in ${import_chalk79.default.bold(contextName)}:`;
  output_manager_default.log(
    `${headerMessage}
${table(
      [
        ["Name", "Status", "Product", "Integration", "Projects"].map(
          (header) => import_chalk79.default.bold(import_chalk79.default.cyan(header))
        ),
        ...results.map((result) => [
          resourceLink(contextName, result) ?? import_chalk79.default.gray("\u2013"),
          resourceStatus(result.status ?? "\u2013"),
          result.product ?? import_chalk79.default.gray("\u2013"),
          integrationLink(result, team) ?? import_chalk79.default.gray("\u2013"),
          import_chalk79.default.grey(
...
function resourceStatus(status3) {
  ...
  switch (status3) {
    case "initializing":
      return import_chalk79.default.yellow(CIRCLE) + statusTitleCase;
    case "error":
      return import_chalk79.default.red(CIRCLE) + statusTitleCase;
    case "available":
      return import_chalk79.default.green(CIRCLE) + statusTitleCase;
    case "suspended":
      return import_chalk79.default.white(CIRCLE) + statusTitleCase;
    case "limits-exceeded-suspended":
      return `${import_chalk79.default.white(CIRCLE)}Limits exceeded`;
    default:
      return import_chalk79.default.gray(statusTitleCase);
  }
}
...
function integrationLink(integration, team) {
  ...
  const boldName = import_chalk79.default.bold(integration.integration);
...
async function openIntegration(client, subArgs) {
  ...
  const flagsSpecification = getFlagsSpecification(openSubcommand2.options);
  let parsedArguments;
  try {
  ...
  try {
    ...
    output_manager_default.error(
      `Failed to fetch configuration for ${import_chalk80.default.bold(`"${integrationSlug}"`)}: ${error.message}`
    );
    return 1;
  } finally {
  ...
  if (!configuration) {
    output_manager_default.error(
      `No configuration found for ${import_chalk80.default.bold(`"${integrationSlug}"`)}.`
    );
    return 1;
  }
  ...
  if (resourceName) {
    ...
    if (!resource) {
      output_manager_default.error(
        `Resource ${import_chalk80.default.bold(`"${resourceName}"`)} not found for integration ${import_chalk80.default.bold(`"${integrationSlug}"`)}.`
      );
      return 1;
    }
...
`);
    ...
    const label = isResource ? `Opening the ${import_chalk80.default.bold(name)} resource dashboard...` : `Opening the ${import_chalk80.default.bold(name)} dashboard...`;
    output_manager_default.print(label);
    (0, import_open8.default)(link);
  } else {
...
async function remove(client) {
  ...
  if (!integrationConfiguration) {
    output_manager_default.error(`No integration ${import_chalk81.default.bold(integrationName)} found.`);
...
`
      ...
      output_manager_default.error(
        `Cannot uninstall ${import_chalk81.default.bold(integrationName)} because it still has resources.`
      );
      if (resourceNames.length > 0) {
        output_manager_default.log("");
        output_manager_default.log("Resources that must be removed first:");
        for (const name of resourceNames) {
          output_manager_default.log(`  ${import_chalk81.default.gray("-")} ${name}`);
        }
        output_manager_default.log("");
      }
      if (client.isAgent) {
      ...
      output_manager_default.log(
        `Remove and disconnect all resources first with: ${import_chalk81.default.cyan(`${packageName} integration-resource remove <resource-name> --disconnect-all`)}`
      );
      output_manager_default.log(
        `Then retry: ${import_chalk81.default.cyan(`${packageName} integration remove ${integrationName}`)}`
      );
      return 1;
    }
    output_manager_default.error(
      import_chalk81.default.red(
        `Failed to remove ${import_chalk81.default.bold(integrationName)}: ${error.message}`
      )
    );
    return 1;
  }
...
`
  ...
  output_manager_default.success(`${import_chalk81.default.bold(integrationName)} successfully removed.`);
  return 0;
}
async function confirmIntegrationRemoval(client, integration, team) {
  output_manager_default.log(
    `The ${import_chalk81.default.bold(integration)} integration will be removed permanently from team ${import_chalk81.default.bold(team.name)}.`
  );
  ...
  return table(
    [
      ["Product Name", "Slug", "Provider", "Description"].map(
        (header) => import_chalk82.default.bold(import_chalk82.default.cyan(header))
      ),
      ...products.map((product) => [
        product.name,
        product.slug,
        product.provider,
        product.description || import_chalk82.default.gray("-")
      ])
    ],
    { hsep: 4 }
  );
...
function formatCompactList(products) {
  return products.map((product) => {
    return [
      `${import_chalk82.default.bold(product.name)} (${product.slug})`,
    ...
    if (!product) {
      ...
      if (products.length > 1) {
        ...
        for (const p of products) {
          output_manager_default.log(`  ${import_chalk83.default.cyan(p.slug)}  ${p.name}`);
        }
      }
      return 1;
    }
    ...
    if (frameworkFlag) {
      ...
      if (!selectedGuide) {
        ...
        for (const g of guides) {
          output_manager_default.log(`  ${import_chalk83.default.cyan(g.framework)}  ${g.title}`);
        }
        return 1;
      }
...
var COMMAND_CONFIG17 = {
  ...
  open: getCommandAliases(openSubcommand2),
...
async function createThreshold(client) {
  ...
  output_manager_default.stopSpinner();
  if (!targetedResource) {
    output_manager_default.error(`The resource ${import_chalk84.default.bold(resourceName)} was not found.`);
    return 1;
  }
  if (!targetedResource.product?.integrationConfigurationId) {
    output_manager_default.error(
      `The resource ${import_chalk84.default.bold(resourceName)} does not have an integration configuration.`
    );
    return 1;
  }
  if (targetedResource.billingPlan?.type !== "prepayment") {
    output_manager_default.error(
      `The resource ${import_chalk84.default.bold(resourceName)} is not a prepayment-based resource.`
    );
    return 1;
  }
  ...
  if (targetedResource.billingPlan.scope !== "resource") {
    output_manager_default.log(
      `The resource ${import_chalk84.default.bold(resourceName)} uses an installation-level balance.`
    );
...
async function handleUpdateThreshold(props) {
  if (props.resource.billingPlan?.type !== "prepayment") {
    output_manager_default.log(
      `The resource ${import_chalk84.default.bold(props.resource.name)} is not a prepayment-based resource.`
    );
    return 0;
  }
  if (!props.resource.product?.integrationConfigurationId) {
    output_manager_default.log(
      `The resource ${import_chalk84.default.bold(props.resource.name)} does not have an integration configuration.`
    );
    return 0;
  }
  ...
  const entityTextReference = props.isInstallationLevel ? `installation ${import_chalk84.default.bold(props.resource.product?.name)}` : `resource ${import_chalk84.default.bold(props.resource.name)}`;
...
async function disconnect2(client) {
  ...
  output_manager_default.stopSpinner();
  if (!targetedResource) {
    output_manager_default.error(`No resource ${import_chalk85.default.bold(resourceName)} found.`);
    return 1;
  }
  if (parsedArguments.flags["--all"]) {
    try {
...
async function handleDisconnectProject(client, resource, projectName, skipConfirmation, asJson) {
  ...
  if (!project) {
    output_manager_default.log(
      `Could not find project ${import_chalk85.default.bold(projectName)} connected to resource ${import_chalk85.default.bold(resource.name)}.`
    );
    return 0;
  }
  if (!skipConfirmation && !client.stdin.isTTY) {
...
`
  ...
  output_manager_default.success(
    `Disconnected ${import_chalk85.default.bold(project.name)} from ${import_chalk85.default.bold(resource.name)}`
  );
  return 0;
}
async function handleDisconnectAllProjects(client, resource, skipConfirmation, asJson = false) {
  if (resource.projectsMetadata?.length === 0) {
    output_manager_default.log(`${import_chalk85.default.bold(resource.name)} has no projects to disconnect.`);
    return;
  }
  if (!skipConfirmation && !client.stdin.isTTY) {
  ...
  try {
    ...
    if (asJson) {
      ...
      output_manager_default.success(
        `Disconnected all projects from ${import_chalk85.default.bold(resource.name)}`
      );
    }
  } catch (error) {
    throw new FailedError(
...
async function confirmDisconnectProject(client, resource, project) {
  output_manager_default.log(
    `The resource ${import_chalk85.default.bold(resource.name)} will be disconnected from project ${import_chalk85.default.bold(project.name)}.`
  );
  return client.input.confirm(`${import_chalk85.default.red("Are you sure?")}`, false);
}
...
async function remove2(client) {
  ...
  output_manager_default.stopSpinner();
  if (!targetedResource) {
    output_manager_default.error(`No resource ${import_chalk86.default.bold(resourceName)} found.`);
    return 1;
  }
  if (disconnectAll) {
    try {
...
async function handleDeleteResource(client, resource, options) {
  ...
  if (!options?.skipProjectCheck && hasProjects) {
    output_manager_default.error(
      `Cannot delete resource ${import_chalk86.default.bold(resource.name)} while it has connected projects. Please disconnect any projects using this resource first or use the \`--disconnect-all\` flag.`
    );
    return 1;
  }
  ...
  try {
    ...
    output_manager_default.error(
      `A problem occurred when attempting to delete ${import_chalk86.default.bold(resource.name)}: ${error.message}`
    );
    return 1;
  }
  if (options?.asJson) {
...
`
  ...
  output_manager_default.success(`${import_chalk86.default.bold(resource.name)} successfully deleted.`);
  return 0;
}
async function confirmDeleteResource(client, resource) {
  output_manager_default.log(`${import_chalk86.default.bold(resource.name)} will be deleted permanently.`);
  return client.input.confirm(`${import_chalk86.default.red("Are you sure?")}`, false);
}
...
async function logs(client) {
  ...
  if (followOption) {
    ...
    const usedIncompatible = incompatibleFlags.filter((f) => f.value !== void 0 && f.value !== null).map((f) => import_chalk87.default.bold(f.flag));
    if (usedIncompatible.length > 0) {
      output_manager_default.error(
        `The ${import_chalk87.default.bold("--follow")} flag does not support filtering. Remove: ${usedIncompatible.join(", ")}`
      );
      return 1;
    }
  }
    ...
    if (link.status === "error") {
      ...
      output_manager_default.error(
        `Your codebase isn't linked to a project on Vercel. Run ${getCommandName(
          "link"
        )} to begin, or specify a project with ${import_chalk87.default.bold("--project")}.`
      );
      return 1;
    }
    client.config.currentTeam = link.org.type === "team" ? link.org.id : void 0;
  ...
  if (followOption) {
    if (!deploymentId) {
      if (noBranchFlagValue) {
        output_manager_default.error(
          `The ${import_chalk87.default.bold("--follow")} flag requires a deployment. Specify one with ${import_chalk87.default.bold("--deployment")} or remove ${import_chalk87.default.bold("--no-branch")} to auto-detect from the current git branch.`
        );
        return 1;
      }
      if (!branchOption) {
        output_manager_default.error(
          `The ${import_chalk87.default.bold("--follow")} flag requires a deployment. Specify one with ${import_chalk87.default.bold("--deployment")} or run from within a git repository.`
        );
        return 1;
      }
      output_manager_default.spinner(
      ...
      if (!branchDeployment) {
        output_manager_default.error(
          `No deployments found for branch "${branchOption}". Deploy this branch first or specify a deployment with ${import_chalk87.default.bold("--deployment")}.`
        );
        return 1;
      }
    ...
    if (!jsonOption) {
      output_manager_default.print(
        `Streaming logs for deployment ${import_chalk87.default.bold(deploymentId)} starting from ${import_chalk87.default.bold((0, import_date_fns2.format)(Date.now(), TIME_ONLY_FORMAT))}
...
`
  ...
  output_manager_default.stopSpinner();
  if (!jsonOption) {
    const branchSuffix = branchOption ? ` on branch ${import_chalk87.default.cyan(branchOption)}` : "";
    if (logs2.length === 0) {
      output_manager_default.print(
        import_chalk87.default.dim(
...
`
      ...
      const baseColumns = [
        {
          ...
          format: (padded) => import_chalk87.default.dim(padded)
        },
        {
          label: "HOST",
      ...
      const columns = expandOption ? baseColumns : [
        ...baseColumns,
        {
          ...
          format: (padded, row) => row.statusCode <= 0 ? import_chalk87.default.gray(padded) : colorizeStatus(padded, row.statusCode)
        },
        {
          label: "MESSAGE",
          width: "stretch",
          getValue: (row) => row.message || "(no message)",
          format: (padded, row) => row.message ? colorizeMessage(padded, row.level) : import_chalk87.default.dim(padded)
        }
      ];
      const formatted = table2({
        ...
        formatHeader: (header) => import_chalk87.default.dim(header),
        formatRow: expandOption ? (rowStr, row) => {
          if (row.message) {
            const coloredMessage = colorizeMessage(row.message, row.level);
            const truncatedIndicator = row.messageTruncated ? import_chalk87.default.gray("\u2026") : "";
            return `${rowStr}
...
function colorizeLevel(formatted, level) {
  switch (level) {
    case "fatal":
      return import_chalk87.default.red.bold(formatted);
    case "error":
      return import_chalk87.default.red(formatted);
    case "warning":
      return import_chalk87.default.yellow(formatted);
    default:
      return import_chalk87.default.dim(formatted);
  }
}
function colorizeStatus(formatted, statusCode) {
  if (statusCode >= 500) {
    ...
    return import_chalk87.default.cyan(formatted);
  } else if (statusCode >= 200) {
    return import_chalk87.default.green(formatted);
  }
  return import_chalk87.default.gray(formatted);
}
...
function colorizeMessage(message, level) {
  switch (level) {
    case "fatal":
    case "error":
      return import_chalk87.default.red(message);
    case "warning":
      return import_chalk87.default.yellow(message);
    default:
      return import_chalk87.default.dim(message);
  }
}
...
async function metrics(client) {
  ...
  switch (subcommand) {
    ...
    default: {
      ...
      const queryFn = (await import("./chunks/query-DIK6TLAL.js")).default;
...
async function pull2(client) {
  ...
  const { contextName } = await getScope(client);
  output_manager_default.spinner(
    `Fetching microfrontends configuration in ${import_chalk88.default.bold(contextName)}`
  );
  let rawConfig;
  ...
  try {
    ...
    output_manager_default.print(
      `${prependEmoji(
        `Downloaded microfrontends configuration to ${import_chalk88.default.bold(
          humanizePath(
            join4(currentDirectory, VERCEL_DIR, VERCEL_DIR_MICROFRONTENDS)
          )
        )} ${import_chalk88.default.gray(microfrontendsStamp())}`,
        emoji("success")
      )}
`
    );
...
async function openCommandHandler(client) {
  ...
  await (0, import_open10.default)(projectUrl);
  return 0;
}
...
async function add6(client, argv) {
  ...
  if (args.length !== 1) {
    ...
    if (args.length > 1) {
      const example = import_chalk89.default.cyan(
  ...
  const { contextName } = await getScope(client);
  output_manager_default.log(
    `${import_chalk89.default.cyan("Success!")} Project ${import_chalk89.default.bold(
      name.toLowerCase()
    )} added (${import_chalk89.default.bold(contextName)}) ${import_chalk89.default.gray(`[${elapsed2}]`)}`
  );
  return 0;
}
...
async function inspect4(client, argv) {
  ...
  const projectSlugLink = formatProject(org.slug, project.name);
  output_manager_default.log(`Found Project ${projectSlugLink} ${import_chalk90.default.gray(inspectStamp())}`);
  output_manager_default.print("\n");
  output_manager_default.print(import_chalk90.default.bold("  General\n\n"));
  output_manager_default.print(`    ${import_chalk90.default.cyan("ID")}				${project.id}
`);
  output_manager_default.print(`    ${import_chalk90.default.cyan("Name")}			${project.name}
`);
  output_manager_default.print(`    ${import_chalk90.default.cyan("Owner")}			${org.name}
`);
  output_manager_default.print(
    `    ${import_chalk90.default.cyan("Created At")}			${formatDate(project.createdAt)}
`
  );
  output_manager_default.print(
    `    ${import_chalk90.default.cyan("Root Directory")}		${project.rootDirectory ?? "."}
`
  );
  output_manager_default.print(
    `    ${import_chalk90.default.cyan("Node.js Version")}		${project.nodeVersion}
`
  );
  const framework = import_frameworks.frameworkList.find((f) => f.slug === project.framework);
  output_manager_default.print("\n");
  output_manager_default.print(import_chalk90.default.bold("  Framework Settings\n\n"));
  output_manager_default.print(`    ${import_chalk90.default.cyan("Framework Preset")}		${framework?.name}
`);
  output_manager_default.print(
    `    ${import_chalk90.default.cyan("Build Command")}		${project.buildCommand ?? import_chalk90.default.dim(framework?.settings?.buildCommand.placeholder ?? "None")}
`
  );
  output_manager_default.print(
    `    ${import_chalk90.default.cyan("Output Directory")}		${project.outputDirectory ?? import_chalk90.default.dim(framework?.settings?.outputDirectory.placeholder ?? "None")}
`
  );
  output_manager_default.print(
    `    ${import_chalk90.default.cyan("Install Command")}		${project.installCommand ?? import_chalk90.default.dim(framework?.settings?.installCommand.placeholder ?? "None")}
`
  );
  output_manager_default.print("\n");
  return 0;
...
async function list5(client, argv) {
  ...
  const start = Date.now();
  const { contextName } = await getScope(client);
  output_manager_default.spinner(`Fetching projects in ${import_chalk91.default.bold(contextName)}`);
...
function printProjectsTable(projectList) {
  const tablePrint = table(
    [
      TABLE_HEADERS.map((header) => import_chalk91.default.bold(import_chalk91.default.cyan(header))),
      ...projectList.flatMap((project) => [
        [
          import_chalk91.default.bold(project.name),
          getLatestProdUrl(project),
          import_chalk91.default.gray((0, import_ms16.default)(Date.now() - project.updatedAt)),
          project.nodeVersion ?? ""
        ]
      ])
    ],
...
async function rm6(client, argv) {
  ...
  output_manager_default.log(
    `${import_chalk92.default.cyan("Success!")} Project ${import_chalk92.default.bold(name)} removed ${import_chalk92.default.gray(
      `[${elapsed2}]`
    )}`
  );
  return 0;
...
async function readConfirmation3(client, projectName) {
  output_manager_default.print(
    prependEmoji(
      `The project ${import_chalk92.default.bold(projectName)} will be removed permanently.
...
`,
  ...
  return await client.input.confirm(
    `${import_chalk92.default.bold.red("Are you sure?")}`,
    false
  );
}
...
async function getProjectByDeployment({
  ...
  try {
    output_manager_default?.spinner(
      `Fetching deployment "${deployId}" in ${import_chalk94.default.bold(contextName)}\u2026`
    );
    ...
    if (deployment.team?.id) {
      if (!team || deployment.team.id !== team.id) {
        const err = new Error(
          team ? `Deployment doesn't belong to current team ${import_chalk94.default.bold(
            contextName
      ...
      const err = new Error(
        `Deployment doesn't belong to current team ${import_chalk94.default.bold(contextName)}`
      );
      err.code = "ERR_INVALID_TEAM";
      throw err;
    }
...
function renderAliasStatus(status3) {
  if (status3 === "completed") {
    return import_chalk95.default.green(status3);
  }
  if (status3 === "failed") {
    return import_chalk95.default.red(status3);
  }
  if (status3 === "skipped") {
    return import_chalk95.default.gray(status3);
  }
  return import_chalk95.default.yellow(status3);
}
  ...
  try {
    ...
    for (; ; ) {
      ...
      if (requestedAt < recentThreshold || Date.now() >= promoteTimeout) {
        output_manager_default.log(
          `The promotion exceeded its deadline - rerun ${import_chalk96.default.bold(
...
async function renderJobSucceeded({
  ...
  try {
    ...
    deploymentInfo = `${import_chalk96.default.bold(deployment.url)} (${toDeploymentId})`;
  } catch (err) {
    output_manager_default.debug(
      `Failed to get deployment url for ${toDeploymentId}: ${err?.toString() || err}`
    );
    deploymentInfo = import_chalk96.default.bold(toDeploymentId);
  }
  const duration = performingPromote ? elapsed(Date.now() - requestedAt) : "";
  output_manager_default.log(
    `Success! ${import_chalk96.default.bold(
      project.name
...
async function requestPromote({
  ...
  if (promoteByCreation) {
    ...
    output_manager_default.log(
      `Successfully created new deployment of ${import_chalk97.default.bold(project.name)} at ${newDeployment.inspectorUrl}`
    );
    return 0;
  }
  const res = await client.fetch(
  ...
  if (timeout !== void 0 && (0, import_ms19.default)(timeout) === 0) {
    output_manager_default.log(
      `Successfully requested promote of ${import_chalk97.default.bold(project.name)} to ${deployment.url} (${deployment.id})`
    );
...
async function getDeploymentByIdOrURL({
  ...
  try {
    output_manager_default.spinner(
      `Fetching deployment "${deployIdOrUrl}" in ${import_chalk98.default.bold(contextName)}\u2026`
    );
    const [teamResult, deploymentResult] = await Promise.allSettled([
      config.currentTeam ? getTeamById(client, config.currentTeam) : void 0,
...
async function redeploy(client) {
  ...
  try {
    ...
    output_manager_default.print(
      `${prependEmoji(
        `Inspect: ${import_chalk99.default.bold(deployment.inspectorUrl)} ${deployStamp()}`,
        emoji("inspect")
      )}
`
    );
    output_manager_default.print(
      prependEmoji(
        `${isProdDeployment ? "Production" : "Preview"}: ${import_chalk99.default.bold(
          previewUrl
        )} ${deployStamp()}`,
        emoji("success")
      ) + `
          ...
          for await (const event of (0, import_client.checkDeploymentStatus)(
            ...
            if (event.type === "building") {
              ...
              if (event.type === "alias-assigned" && !Array.isArray(event.payload) && event.payload.target === "production" && event.payload.alias && event.payload.alias.length > 0) {
                ...
                output_manager_default.print(
                  prependEmoji(
                    `Aliased: ${import_chalk99.default.bold(prodUrl)} ${deployStamp()}`,
                    emoji("link")
                  ) + "\n"
                );
              }
...
async function list6(client, argv) {
  ...
  if (staging) {
    ...
    if (!stagingVersion) {
      output_manager_default.error(
        `No staging version found for ${import_chalk100.default.bold(project.name)}. Run ${import_chalk100.default.cyan(
  ...
  if (versionIdFlag) {
    ...
    if (!version) {
      output_manager_default.error(
        `Version "${versionIdFlag}" not found. Run ${import_chalk100.default.cyan(
          "vercel redirects list-versions"
        )} to see available versions.`
      );
      return 1;
  ...
  const lsStamp = stamp_default();
  let spinnerMessage = `Fetching redirects for ${import_chalk100.default.bold(project.name)}`;
  if (versionName) {
  ...
  if (useDiff) {
    ...
    output_manager_default.log(
      `Changes in staging version ${import_chalk100.default.bold(versionName || "")} ${import_chalk100.default.gray(lsStamp())}`
    );
    if (added.length === 0 && removed.length === 0) {
      ...
      if (added.length > 0) {
        output_manager_default.print(`
  ${import_chalk100.default.bold(import_chalk100.default.green(`Added (${added.length}):`))}
`);
        output_manager_default.print(formatRedirectsTable(added, "+"));
      }
      if (removed.length > 0) {
        output_manager_default.print(`
  ${import_chalk100.default.bold(import_chalk100.default.red(`Removed (${removed.length}):`))}
`);
      ...
      if (unchanged.length > 0) {
        output_manager_default.print(
          `
  ${import_chalk100.default.gray(`${unchanged.length} redirect${unchanged.length === 1 ? "" : "s"} unchanged`)}
`
        );
      }
      output_manager_default.print("\n");
    ...
    let resultMessage = `${(0, import_pluralize9.default)("Redirect", redirects.length, true)} found for ${import_chalk100.default.bold(
      project.name
    )}`;
    if (versionName) {
      resultMessage += ` ${import_chalk100.default.gray(`(version: ${versionName})`)}`;
    }
    if (search) {
      resultMessage += ` matching "${search}"`;
    }
    if (pagination) {
      resultMessage += ` ${import_chalk100.default.gray(`(page ${pagination.page} of ${pagination.numPages})`)}`;
    }
...
function formatRedirectsTable(redirects, actionSymbol) {
  const rows = redirects.map((redirect) => {
    ...
    const colorFn = actionSymbol === "+" ? import_chalk100.default.green : actionSymbol === "-" ? import_chalk100.default.red : (s) => s;
    return [
...
async function listVersions(client, argv) {
  ...
  output_manager_default.log(
    `${(0, import_pluralize10.default)("Version", sortedVersions.length, true)} found for ${import_chalk101.default.bold(
      project.name
...
function formatVersionsTable(versions) {
  ...
  const rows = versions.map((version) => {
    ...
    if (version.isStaging) {
      status3 = import_chalk101.default.yellow("Staging");
    } else if (version.isLive) {
      status3 = import_chalk101.default.green("Live");
    } else {
      status3 = import_chalk101.default.gray("Previous");
    }
    const name = version.name || import_chalk101.default.gray("(unnamed)");
    const redirectCountStr = version.redirectCount !== void 0 && version.redirectCount !== null ? version.redirectCount.toString() : import_chalk101.default.gray("-");
    return [
      status3,
      name,
      version.id,
      redirectCountStr,
      version.createdBy || import_chalk101.default.gray("(unknown)"),
      import_chalk101.default.gray(age + " ago")
    ];
  });
  return formatTable(
...
`);
  output_manager_default.print(`    ${import_chalk102.default.cyan(source)} \u2192 ${import_chalk102.default.cyan(destination)}
`);
...
`
  ...
  if (alias2) {
    ...
    output_manager_default.print(
      `
  ${import_chalk102.default.bold("Test your changes:")} ${import_chalk102.default.cyan(testUrl)}
`
    );
  }
  const newVersionName = version.name || version.id;
  output_manager_default.print(`  ${import_chalk102.default.bold("New staging version:")} ${newVersionName}
...
`);
  if (!existingStagingVersion) {
    ...
    if (shouldPromote) {
      ...
      output_manager_default.log(
        `${import_chalk102.default.cyan("\u2713")} Version promoted to production ${import_chalk102.default.gray(promoteStamp())}`
      );
    }
  } else {
    output_manager_default.warn(
      `There are other staged changes. Please review all changes with ${import_chalk102.default.cyan("vercel redirects list --staging")} before promoting to production.`
    );
  }
  return 0;
}
...
async function upload(client, argv) {
  ...
  try {
    ...
    output_manager_default.print(`
  ${import_chalk103.default.bold("Summary:")}
`);
    output_manager_default.print(
...
`
    ...
    if (redirectCount > 0) {
      ...
      output_manager_default.print(`
  ${import_chalk103.default.bold("Changes:")}
`);
      if (added.length > 0) {
        output_manager_default.print(`    ${import_chalk103.default.green(`Added: ${added.length}`)}
`);
      }
      if (deleted.length > 0) {
        output_manager_default.print(`    ${import_chalk103.default.red(`Deleted: ${deleted.length}`)}
`);
      }
      if (edited.length > 0) {
        output_manager_default.print(`    ${import_chalk103.default.yellow(`Modified: ${edited.length}`)}
`);
      }
      output_manager_default.print(`
  ${import_chalk103.default.bold("Redirect changes:")}
`);
      ...
      const rows = displayRedirects.map((redirect) => {
        ...
        switch (action) {
          case "+":
            colorFn = import_chalk103.default.green;
            actionSymbol = "+";
            break;
          case "-":
            colorFn = import_chalk103.default.red;
            actionSymbol = "-";
            break;
          case "~":
            colorFn = import_chalk103.default.yellow;
            actionSymbol = "~";
            break;
          default:
            colorFn = (s) => s;
      ...
      if (redirectCount > 100) {
        output_manager_default.print(
          `
  ${import_chalk103.default.gray(`... and ${redirectCount - 100} more redirect${redirectCount - 100 === 1 ? "" : "s"}`)}
`
        );
      }
    }
    if (result.alias) {
      ...
      output_manager_default.print(
        `
  ${import_chalk103.default.bold("Test your changes:")} ${import_chalk103.default.cyan(testUrl)}
`
      );
    }
    const newVersionName = result.version.name || result.version.id;
    output_manager_default.print(
      `  ${import_chalk103.default.bold("New staging version:")} ${newVersionName}
...
async function remove3(client, argv) {
  ...
  output_manager_default.print(`
  ${import_chalk104.default.bold("Removing redirect:")}
`);
  output_manager_default.print(
    `    ${import_chalk104.default.cyan(redirectToRemove.source)} \u2192 ${import_chalk104.default.cyan(redirectToRemove.destination)}
`
  );
...
`);
  ...
  const removeStamp = stamp_default();
  output_manager_default.spinner(`Removing redirect for ${import_chalk104.default.bold(source)}`);
  ...
  if (alias2) {
    ...
    output_manager_default.print(
      `
  ${import_chalk104.default.bold("Test your changes:")} ${import_chalk104.default.cyan(testUrl)}
`
    );
    output_manager_default.print(
...
`
  ...
  output_manager_default.print(`  ${import_chalk104.default.bold("New staging version:")} ${versionName}
...
`);
  if (!existingStagingVersion) {
    ...
    if (shouldPromote) {
      ...
      output_manager_default.log(
        `${import_chalk104.default.cyan("\u2713")} Version promoted to production ${import_chalk104.default.gray(promoteStamp())}`
      );
    }
  } else {
    output_manager_default.warn(
      `There are other staged changes. Review them with ${import_chalk104.default.cyan("vercel redirects list --staging")} before promoting to production.`
    );
  }
  return 0;
}
...
async function promote(client, argv) {
  ...
  output_manager_default.spinner(`Fetching redirect versions for ${import_chalk105.default.bold(project.name)}`);
  const { versions } = await getRedirectVersions(client, project.id, teamId);
  const version = versions.find(
...
Run ${import_chalk105.default.cyan(
        "vercel redirects list-versions"
  ...
  if (changedRedirects.length > 0) {
    output_manager_default.print(`
${import_chalk105.default.bold("Changes to be promoted:")}

`);
    const displayRedirects = changedRedirects.slice(0, 20);
    for (const redirect of displayRedirects) {
      const status3 = redirect.statusCode || (redirect.permanent ? 308 : 307);
      const symbol = redirect.action === "+" ? import_chalk105.default.green("+") : import_chalk105.default.red("-");
      output_manager_default.print(
...
`)
    ...
    output_manager_default.print(
      `
${import_chalk105.default.gray("No changes detected from current production version.")}
...
`
  ...
  const confirmed = await confirmAction(
    client,
    parsed.flags["--yes"],
    `Promote version ${import_chalk105.default.bold(versionName)} to production?`,
    `This will make it the live version for ${import_chalk105.default.bold(project.name)}.`
  );
  if (!confirmed) {
    output_manager_default.log("Canceled");
    return 0;
  ...
  output_manager_default.spinner(`Promoting version ${import_chalk105.default.bold(versionName)} to production`);
  ...
  output_manager_default.log(
    `${import_chalk105.default.cyan("\u2713")} Version ${import_chalk105.default.bold(
      newVersion.name || newVersion.id
    )} promoted to production ${import_chalk105.default.gray(updateStamp())}`
  );
  return 0;
}
...
async function restore(client, argv) {
  ...
  const teamId = org.type === "team" ? org.id : void 0;
  const [versionIdentifier] = parsed.args;
  output_manager_default.spinner(`Fetching redirect versions for ${import_chalk106.default.bold(project.name)}`);
  const { versions } = await getRedirectVersions(client, project.id, teamId);
  const version = versions.find(
    (v) => v.id === versionIdentifier || v.name === versionIdentifier
  );
...
Run ${import_chalk106.default.cyan(
        "vercel redirects list-versions"
  ...
  if (changedRedirects.length > 0) {
    output_manager_default.print(`
${import_chalk106.default.bold("Changes to be restored:")}

`);
    const displayRedirects = changedRedirects.slice(0, 20);
    for (const redirect of displayRedirects) {
      const status3 = redirect.statusCode || (redirect.permanent ? 308 : 307);
      const symbol = redirect.action === "+" ? import_chalk106.default.green("+") : import_chalk106.default.red("-");
      output_manager_default.print(
        `  ${symbol} ${redirect.source} \u2192 ${redirect.destination} (${status3})
`
      );
...
`)
    ...
    output_manager_default.print(
      `
${import_chalk106.default.gray("No changes detected from current production version.")}
...
`
  ...
  const confirmed = await confirmAction(
    client,
    parsed.flags["--yes"],
    `Restore version ${import_chalk106.default.bold(versionName)}?`,
    `This will make it the live version for ${import_chalk106.default.bold(project.name)}.`
  );
  if (!confirmed) {
    output_manager_default.log("Canceled");
    return 0;
  ...
  const updateStamp = stamp_default();
  output_manager_default.spinner(`Restoring version ${import_chalk106.default.bold(versionName)}`);
  const { version: newVersion } = await updateRedirectVersion(
    client,
    project.id,
    version.id,
  ...
  output_manager_default.log(
    `${import_chalk106.default.cyan("\u2713")} Version ${import_chalk106.default.bold(
      newVersion.name || newVersion.id
    )} restored to production ${import_chalk106.default.gray(updateStamp())}`
  );
  return 0;
}
...
async function remove4(client) {
  ...
  const { contextName } = await getScope(client);
  output_manager_default.spinner(
    `Fetching deployment(s) ${ids.map((id) => `"${id}"`).join(" ")} in ${import_chalk107.default.bold(contextName)}`
  );
  let aliases;
  let projects;
  let deployments;
  ...
  if (deployments.length === 0 && projects.length === 0) {
    ...
    const stylizedIds = ids.map((id) => import_chalk107.default.bold(`"${id}"`)).join(", ");
  ...
  log(
    `Found ${deploymentsAndProjects(deployments, projects)} for removal in ${import_chalk107.default.bold(contextName)} ${elapsed(Date.now() - findStart)}`
  );
  ...
  deployments.forEach((depl) => {
    output_manager_default.print(`${import_chalk107.default.gray("-")} ${import_chalk107.default.bold(depl.url)}
`);
  });
  projects.forEach((project) => {
    output_manager_default.print(`${import_chalk107.default.gray("-")} ${import_chalk107.default.bold(project.name)}
`);
  });
  return 0;
}
    ...
    if (deployments.length > 0) {
      ...
      const deploymentTable = table(
        deployments.map((depl) => {
          const time = import_chalk107.default.gray(`${(0, import_ms22.default)(Date.now() - depl.createdAt)} ago`);
          const url = depl.url ? import_chalk107.default.underline(`https://${depl.url}`) : "";
...
`);
    ...
    for (const depl of deployments) {
      for (const { alias: alias2 } of depl.aliases) {
        output_manager_default.warn(
          `${import_chalk107.default.underline(`https://${alias2}`)} is an alias for ${import_chalk107.default.bold(depl.url)} and will be removed`
        );
      }
    }
...
`);
    ...
    output_manager_default.print(
      `${import_chalk107.default.bold.red("> Are you sure?")} ${import_chalk107.default.gray("(y/N) ")}`
    );
  ...
  try {
    ...
    for (; ; ) {
      ...
      if (requestedAt < recentThreshold || Date.now() >= rollbackTimeout) {
        output_manager_default.log(
          `The rollback exceeded its deadline - rerun ${import_chalk108.default.bold(
...
async function requestRollback({
  ...
  if (timeout !== void 0 && (0, import_ms24.default)(timeout) === 0) {
    output_manager_default.log(
      `Successfully requested rollback of ${import_chalk109.default.bold(project.name)} to ${deployment.url} (${deployment.id})`
    );
...
async function list7(client, argv) {
  ...
  if (diffFlag && !versionIdFlag) {
    ...
    if (!stagingVersion) {
      output_manager_default.error(
        ...
        )} or ${import_chalk110.default.cyan(getCommandName("routes edit"))} to make changes.`
      );
      return 1;
    }
    versionId = stagingVersion.id;
  ...
  const lsStamp = stamp_default();
  let spinnerMessage = `Fetching routes for ${import_chalk110.default.bold(project.name)}`;
  if (versionName) {
    spinnerMessage += ` (version: ${versionName})`;
  }
  if (search) {
  ...
  if (useDiff) {
    ...
    output_manager_default.log(
      `Changes in staging version ${import_chalk110.default.bold(versionName || "")} ${import_chalk110.default.gray(lsStamp())}`
    );
    if (added.length === 0 && removed.length === 0 && modified.length === 0) {
      ...
      if (added.length > 0) {
        output_manager_default.print(
          `
  ${import_chalk110.default.bold(import_chalk110.default.green(`Added (${added.length}):`))}
`
        );
      ...
      if (modified.length > 0) {
        output_manager_default.print(
          `
  ${import_chalk110.default.bold(import_chalk110.default.yellow(`Modified (${modified.length}):`))}
`
        );
      ...
      if (removed.length > 0) {
        output_manager_default.print(
          `
  ${import_chalk110.default.bold(import_chalk110.default.red(`Removed (${removed.length}):`))}
`
        );
      ...
      if (unchanged.length > 0) {
        output_manager_default.print(
          `
  ${import_chalk110.default.gray(`${unchanged.length} route${unchanged.length === 1 ? "" : "s"} unchanged`)}
`
        );
      }
      output_manager_default.print("\n");
    ...
    if (version?.alias) {
      output_manager_default.log(
        `Test your changes: ${import_chalk110.default.cyan(`https://${version.alias}`)}`
      );
    }
  } else {
    let resultMessage = `${(0, import_pluralize12.default)("Route", routes2.length, true)} found for ${import_chalk110.default.bold(
      project.name
    )}`;
    if (versionName) {
      resultMessage += ` ${import_chalk110.default.gray(`(version: ${versionName})`)}`;
    }
    if (search) {
      resultMessage += ` matching "${search}"`;
    }
    ...
    resultMessage += ` ${import_chalk110.default.gray(lsStamp())}`;
    output_manager_default.log(resultMessage);
    if (routes2.length > 0) {
      if (expand) {
    ...
    if (limit && limit.maxRoutes > 0) {
      ...
      if (utilization >= 0.8) {
        const usageColor = utilization >= 1 ? import_chalk110.default.red : import_chalk110.default.yellow;
        output_manager_default.print(
...
function formatRoutesTable(routes2, actionSymbol) {
  const rows = routes2.map((rule, index) => {
    ...
    const status3 = rule.enabled === false ? import_chalk110.default.red("Disabled") : import_chalk110.default.green("Enabled");
    const prefix = actionSymbol || "";
    const colorFn = actionSymbol === "+" ? import_chalk110.default.green : actionSymbol === "-" ? import_chalk110.default.red : actionSymbol === "~" ? import_chalk110.default.yellow : (s) => s;
    const position = actionSymbol ? "" : `${index + 1}`;
    return [
      colorFn(`${prefix} ${position}`).trim(),
      colorFn(
        truncate(rule.name, 30) + (rule.staged ? import_chalk110.default.yellow(" (draft)") : "")
      ),
...
function formatExpandedRoutes(routes2) {
  ...
  routes2.forEach((rule, index) => {
    ...
    const statusText = rule.enabled === false ? import_chalk110.default.red("Disabled") : import_chalk110.default.green("Enabled");
    const stagedText = rule.staged ? import_chalk110.default.yellow(" (staged)") : "";
    lines.push(
      `  ${import_chalk110.default.bold(`${index + 1}. ${rule.name}`)}${stagedText}  ${import_chalk110.default.gray(`[${rule.id}]`)}`
    );
    lines.push(`     ${import_chalk110.default.cyan("Status:")}  ${statusText}`);
    lines.push(`     ${import_chalk110.default.cyan("Type:")}    ${typeLabels}`);
    if (rule.description) {
      lines.push(`     ${import_chalk110.default.cyan("Desc:")}    ${rule.description}`);
    }
    lines.push(`     ${import_chalk110.default.cyan("Source:")}  ${rule.route.src}`);
    lines.push(`     ${import_chalk110.default.cyan("Syntax:")}  ${syntaxLabel}`);
    if (rule.route.dest) {
      lines.push(`     ${import_chalk110.default.cyan("Dest:")}    ${rule.route.dest}`);
    }
    if (rule.route.status) {
      lines.push(`     ${import_chalk110.default.cyan("Code:")}    ${rule.route.status}`);
    }
    if (rule.route.headers && Object.keys(rule.route.headers).length > 0) {
      lines.push(`     ${import_chalk110.default.cyan("Response Headers:")}`);
      for (const [key, value] of Object.entries(rule.route.headers)) {
        lines.push(`       ${key}: ${value}`);
      }
    }
    if (rule.route.transforms && rule.route.transforms.length > 0) {
      lines.push(`     ${import_chalk110.default.cyan("Transforms:")}`);
    ...
    if (rule.route.has && rule.route.has.length > 0) {
      lines.push(`     ${import_chalk110.default.cyan("Has conditions:")}`);
    ...
    if (rule.route.missing && rule.route.missing.length > 0) {
      lines.push(`     ${import_chalk110.default.cyan("Does not have conditions:")}`);
...
async function listVersions2(client, argv) {
  ...
  const lsStamp = stamp_default();
  output_manager_default.spinner(`Fetching route versions for ${import_chalk111.default.bold(project.name)}`);
  const { versions } = await getRouteVersions(client, project.id, {
    teamId,
    count
  });
  output_manager_default.log(
    `Route versions for ${import_chalk111.default.bold(project.name)} ${import_chalk111.default.gray(lsStamp())}`
  );
...
function formatVersionsTable2(versions) {
  const rows = versions.map((version) => {
    let status3 = "";
    if (version.isStaging) {
      status3 = import_chalk111.default.yellow("Staging");
    } else if (version.isLive) {
      status3 = import_chalk111.default.green("Live");
    } else {
      status3 = import_chalk111.default.gray("Previous");
    }
...
async function inspect5(client, argv) {
  ...
  const inspectStamp = stamp_default();
  output_manager_default.spinner(
    `Searching for route "${identifier}" in ${import_chalk112.default.bold(project.name)}`
  );
  ...
  if (exactMatch) {
    ...
    const selectedId = await client.input.select({
      ...
      choices: routes2.map((r) => ({
        value: r.id,
        name: `${r.name} ${import_chalk112.default.gray(`(${r.route.src})`)}`
      }))
    });
    if (!selectedId) {
  ...
  output_manager_default.log(
    `Route found in ${import_chalk112.default.bold(project.name)} ${import_chalk112.default.gray(inspectStamp())}`
  );
  if (showDiff) {
...
async function showRouteDiff(client, projectId, teamId, stagingRoute) {
  ...
  if (!productionRoute) {
    output_manager_default.print(
      `
  ${import_chalk112.default.bold(stagingRoute.name)} ${import_chalk112.default.green("(new)")}
`
    );
    output_manager_default.print(`  ${import_chalk112.default.gray(stagingRoute.id)}
`);
    output_manager_default.print(
      `  ${import_chalk112.default.green("This route does not exist in production yet.")}
`
    );
    output_manager_default.print(formatRouteDetails(stagingRoute));
    return 0;
...
function diffField(label, oldVal, newVal) {
  if (oldVal === newVal) {
    if (!newVal)
      return "";
    return `  ${import_chalk112.default.cyan(label)} ${newVal}`;
  }
  if (!oldVal && newVal) {
    return `${import_chalk112.default.green("+")} ${import_chalk112.default.cyan(label)} ${newVal}`;
  }
  if (oldVal && !newVal) {
    return `${import_chalk112.default.red("-")} ${import_chalk112.default.cyan(label)} ${import_chalk112.default.strikethrough(oldVal)}`;
  }
  return `${import_chalk112.default.yellow("~")} ${import_chalk112.default.cyan(label)} ${import_chalk112.default.red(oldVal)} \u2192 ${import_chalk112.default.green(newVal)}`;
}
function formatRouteDiff(staging, production) {
  ...
  if (!hasChanges) {
    lines.push(`  ${import_chalk112.default.bold(staging.name)}`);
    lines.push(`  ${import_chalk112.default.gray(staging.id)}`);
    lines.push("");
    lines.push(`  ${import_chalk112.default.gray("No staged changes for this route.")}`);
    lines.push("");
    lines.push(import_chalk112.default.bold("  Route Configuration"));
    return lines.join("\n") + "\n" + formatRouteDetails(staging);
  }
  const typeLabels = getRouteTypeLabel(staging);
  const syntaxLabel = getSrcSyntaxLabel(staging);
  lines.push(`  ${import_chalk112.default.bold(staging.name)} ${import_chalk112.default.yellow("(modified)")}`);
  lines.push(`  ${import_chalk112.default.gray(staging.id)}`);
  lines.push("");
  if (production.description !== staging.description) {
    if (!production.description && staging.description) {
      lines.push(
        `${import_chalk112.default.green("+")} ${import_chalk112.default.cyan("Description:")}  ${staging.description}`
      );
    } else if (production.description && !staging.description) {
      lines.push(
        `${import_chalk112.default.red("-")} ${import_chalk112.default.cyan("Description:")}  ${import_chalk112.default.strikethrough(production.description)}`
      );
    } else {
      lines.push(
        `${import_chalk112.default.yellow("~")} ${import_chalk112.default.cyan("Description:")}  ${import_chalk112.default.red(production.description)} \u2192 ${import_chalk112.default.green(staging.description)}`
      );
    }
    lines.push("");
  ...
  lines.push(
    diffField("Status:", prodStatusText, stagingStatusText) || `  ${import_chalk112.default.cyan("Status:")}      ${stagingStatusText}`
  );
  lines.push(`  ${import_chalk112.default.cyan("Type:")}        ${typeLabels}`);
  lines.push("");
  lines.push(import_chalk112.default.bold("  Route Configuration"));
  const srcLine = diffField("Source:", production.route.src, staging.route.src);
  lines.push(srcLine || `  ${import_chalk112.default.cyan("Source:")}      ${staging.route.src}`);
  lines.push(`  ${import_chalk112.default.cyan("Syntax:")}      ${syntaxLabel}`);
  ...
  if (destLine)
    ...
    lines.push(`  ${import_chalk112.default.cyan("Destination:")} ${staging.route.dest}`);
  ...
  if (statusLine)
    ...
    lines.push(`  ${import_chalk112.default.cyan("HTTP Status:")} ${staging.route.status}`);
  ...
  if (allHeaderKeys.size > 0) {
    lines.push("");
    lines.push(import_chalk112.default.bold("  Response Headers"));
    for (const key of allHeaderKeys) {
      const prodVal = prodHeaders[key];
      const stagingVal = stagingHeaders[key];
      if (prodVal === stagingVal) {
        lines.push(`  ${import_chalk112.default.cyan(key + ":")} ${stagingVal}`);
      } else if (!prodVal) {
        lines.push(
          `${import_chalk112.default.green("+")} ${import_chalk112.default.cyan(key + ":")} ${stagingVal}`
        );
      } else if (!stagingVal) {
        lines.push(
          `${import_chalk112.default.red("-")} ${import_chalk112.default.cyan(key + ":")} ${import_chalk112.default.strikethrough(prodVal)}`
        );
      } else {
        lines.push(
          `${import_chalk112.default.yellow("~")} ${import_chalk112.default.cyan(key + ":")} ${import_chalk112.default.red(prodVal)} \u2192 ${import_chalk112.default.green(stagingVal)}`
        );
      }
    }
  }
  ...
  if (prodTransforms.length > 0 || stagingTransforms.length > 0) {
    lines.push("");
    lines.push(import_chalk112.default.bold("  Transforms"));
    ...
    for (let i = 0; i < stagingTransforms.length; i++) {
      ...
      if (prodIdx === -1) {
        lines.push(
          `${import_chalk112.default.green("+")} ${formatTransform(stagingTransforms[i])}`
        );
      } else {
        ...
        if (prodArgs !== stagingArgs) {
          lines.push(
            `${import_chalk112.default.yellow("~")} ${formatTransform(stagingTransforms[i])} ${import_chalk112.default.gray(`(was: ${prodArgs})`)}`
          );
        } else {
    ...
    for (let i = 0; i < prodTransforms.length; i++) {
      ...
      if (!stagingKeys.includes(key)) {
        lines.push(
          `${import_chalk112.default.red("-")} ${import_chalk112.default.strikethrough(formatTransformPlain(prodTransforms[i]))}`
        );
      }
    }
  }
  ...
  if (prodHas.length > 0 || stagingHas.length > 0) {
    lines.push("");
    lines.push(import_chalk112.default.bold("  Has Conditions"));
  ...
  if (prodMissing.length > 0 || stagingMissing.length > 0) {
    lines.push("");
    lines.push(import_chalk112.default.bold("  Does Not Have Conditions"));
...
function diffConditions(lines, prodConditions, stagingConditions) {
  ...
  for (const cond of stagingConditions) {
    ...
    if (prodKeys.includes(key)) {
      ...
      lines.push(`${import_chalk112.default.green("+")} ${formatCondition(cond)}`);
    }
  }
  for (const cond of prodConditions) {
    const key = conditionKey(cond);
    if (!stagingKeys.includes(key)) {
      lines.push(`${import_chalk112.default.red("-")} ${formatCondition(cond)}`);
    }
  }
}
function formatRouteDetails(rule) {
  ...
  const typeLabels = getRouteTypeLabel(rule);
  const syntaxLabel = getSrcSyntaxLabel(rule);
  const statusText = rule.enabled === false ? import_chalk112.default.red("Disabled") : import_chalk112.default.green("Enabled");
  lines.push(`  ${import_chalk112.default.bold(rule.name)}`);
  lines.push(`  ${import_chalk112.default.gray(rule.id)}`);
  lines.push("");
  if (rule.description) {
    lines.push(`  ${rule.description}`);
    lines.push("");
  }
  lines.push(`  ${import_chalk112.default.cyan("Status:")}      ${statusText}`);
  if (rule.staged === true) {
    lines.push(
      `  ${import_chalk112.default.cyan("State:")}       ${import_chalk112.default.yellow("Staged (not yet published)")}`
    );
  } else if (rule.staged === false) {
    lines.push(`  ${import_chalk112.default.cyan("State:")}       ${import_chalk112.default.green("Published")}`);
  }
  lines.push(`  ${import_chalk112.default.cyan("Type:")}        ${typeLabels}`);
  lines.push("");
  lines.push(import_chalk112.default.bold("  Route Configuration"));
  lines.push(`  ${import_chalk112.default.cyan("Source:")}      ${rule.route.src}`);
  lines.push(`  ${import_chalk112.default.cyan("Syntax:")}      ${syntaxLabel}`);
  if (rule.route.dest) {
    lines.push(`  ${import_chalk112.default.cyan("Destination:")} ${rule.route.dest}`);
  }
  if (rule.route.status) {
    lines.push(`  ${import_chalk112.default.cyan("HTTP Status:")} ${rule.route.status}`);
  }
  ...
  if (responseHeaderSets.length > 0 || responseHeaderTransforms.length > 0) {
    ...
    for (const [key, value] of responseHeaderSets) {
      lines.push(`  ${import_chalk112.default.yellow("set")} ${import_chalk112.default.cyan(key)} = ${value}`);
    }
  ...
  if (requestHeaderTransforms.length > 0) {
    lines.push("");
    lines.push(import_chalk112.default.bold("  Request Headers"));
  ...
  if (requestQueryTransforms.length > 0) {
    lines.push("");
    lines.push(import_chalk112.default.bold("  Request Query"));
...
async function add8(client, argv) {
  ...
  try {
    ...
    output_manager_default.print(`
  ${import_chalk113.default.bold("Route:")} ${route.name}
`);
    output_manager_default.print(`  ${import_chalk113.default.gray("ID:")} ${route.id}
`);
    output_manager_default.print(`  ${import_chalk113.default.gray("Path:")} ${src}
`);
    if (finalDest) {
      if (isRedirect) {
        output_manager_default.print(
          `  ${import_chalk113.default.gray("Redirect:")} ${finalDest} (${finalStatus})
`
        );
      } else {
        output_manager_default.print(`  ${import_chalk113.default.gray("Rewrite:")} ${finalDest}
`);
      }
    } else if (finalStatus) {
      output_manager_default.print(`  ${import_chalk113.default.gray("Status:")} ${finalStatus}
`);
    }
    if (Object.keys(headers).length > 0) {
      output_manager_default.print(
        `  ${import_chalk113.default.gray("Headers:")} ${Object.keys(headers).length} header(s)
`
      );
    }
    if (transforms.length > 0) {
      output_manager_default.print(
        `  ${import_chalk113.default.gray("Transforms:")} ${transforms.length} transform(s)
`
      );
    }
    if (hasConditions.length > 0 || missingConditions.length > 0) {
      output_manager_default.print(
        `  ${import_chalk113.default.gray("Conditions:")} ${hasConditions.length} has, ${missingConditions.length} missing
`
      );
    }
    if (version.alias) {
      ...
      output_manager_default.print(
        `
  ${import_chalk113.default.bold("Test your changes:")} ${import_chalk113.default.cyan(`https://${version.alias}${testPath}`)}
`
      );
    }
    output_manager_default.print(`
  ${import_chalk113.default.bold("Staging version:")} ${version.id}
`);
...
async function main17(client) {
  ...
  switch (subcommand) {
    ...
    case "edit":
      ...
      return (await import("./chunks/edit-AB47LXEE.js")).default(client, args);
    case "delete":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("routes", subcommandOriginal);
      ...
      return (await import("./chunks/delete-DIL4SDG6.js")).default(client, args);
    case "enable":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("routes", subcommandOriginal);
      ...
      telemetry2.trackCliSubcommandEnable(subcommandOriginal);
      return (await import("./chunks/enable-JU6NHQFS.js")).default(client, args);
    case "disable":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("routes", subcommandOriginal);
      ...
      telemetry2.trackCliSubcommandDisable(subcommandOriginal);
      return (await import("./chunks/disable-PZO6GYWY.js")).default(client, args);
    case "reorder":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("routes", subcommandOriginal);
      ...
      return (await import("./chunks/reorder-4VKIZJOA.js")).default(client, args);
    case "export":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("routes", subcommandOriginal);
      ...
      return (await import("./chunks/export-ZKYFXNPW.js")).default(client, args);
    case "publish":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("routes", subcommandOriginal);
      ...
      return (await import("./chunks/publish-UAISKZPC.js")).default(client, args);
    case "restore":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("routes", subcommandOriginal);
      ...
      telemetry2.trackCliSubcommandRestore(subcommandOriginal);
      return (await import("./chunks/restore-5VMKSP2J.js")).default(client, args);
    case "discard-staging":
      ...
      return (await import("./chunks/discard-SXBAOPZJ.js")).default(client, args);
    default:
...
async function autoDetect(client, cwd, asJson, yes) {
  ...
  if (!framework && deps.length === 0) {
    ...
    output_manager_default.log(
      `Try searching directly: ${import_chalk114.default.cyan("vercel skills <query>")}`
    );
    return 0;
  }
  const detectedParts = [];
  ...
  output_manager_default.stopSpinner();
  output_manager_default.log(`Detected: ${import_chalk114.default.bold(detectedParts.join(" + "))}`);
  ...
  if (installable.length === 0) {
    output_manager_default.log(import_chalk114.default.green("All recommended skills are already installed."));
    return 0;
  }
  let toInstall;
  if (yes) {
    ...
    const selected = await client.input.checkbox({
      ...
      choices: installable.map((s) => ({
        name: `${s.skillId} ${import_chalk114.default.gray(`(${s.source})`)}`,
        value: s,
        checked: true
      }))
    });
...
`
  ...
  const tableData = [
    ["Skill", "Installs", "Source", ""].map((h) => import_chalk114.default.bold(import_chalk114.default.cyan(h))),
    ...results.map((s, i) => [
      s.skillId,
      formatInstalls(s.installs),
      import_chalk114.default.gray(s.source),
      installedChecks[i] ? import_chalk114.default.green("installed") : ""
    ])
  ];
  output_manager_default.log(`
...
`);
    ...
    const teamTable = table(
      [
        ["id", "Team name"].map((str) => (0, import_chalk116.gray)(str)),
...
async function change(client, argv) {
  ...
  if (!desiredSlug) {
    const teamChoices = teams2.slice(0).sort((a, b) => {
      ...
      if (selected) {
        title3 += ` ${import_chalk119.default.bold("(current)")}`;
      }
      if (team.limited) {
        title3 += ` ${emoji("locked")}`;
      }
...
async function status2(client) {
  ...
  const status3 = enabled ? import_chalk120.default.green("Enabled") : import_chalk120.default.red("Disabled");
  output_manager_default.print("\n");
  output_manager_default.log(`${import_chalk120.default.bold("Telemetry status")}: ${status3}
`);
...
function outputAggregated({ data, startTime }) {
  ...
  log(
    `Usage for ${import_chalk122.default.bold(data.contextName)} ${elapsed(Date.now() - startTime)}`
  );
  log("");
  const periodSuffix = data.usingDefaults ? " (current month)" : "";
  log(
    `${import_chalk122.default.gray("Period:")} ${data.fromDisplay} to ${data.toDisplay}${periodSuffix}`
  );
  log(`${import_chalk122.default.gray("Charges processed:")} ${data.chargeCount}`);
  log(`${import_chalk122.default.gray("Pricing unit:")} ${data.pricingUnit}`);
  log("");
  ...
  rows.push([
    import_chalk122.default.bold("Total"),
    import_chalk122.default.bold(
      formatQuantity(data.grandTotals.pricingQuantity, data.pricingUnit)
    ),
    import_chalk122.default.bold(formatCurrency(data.grandTotals.effectiveCost)),
    import_chalk122.default.bold(formatCurrency(data.grandTotals.billedCost))
  ]);
  const tablePrint = table(
    [headers.map((h) => import_chalk122.default.bold(import_chalk122.default.cyan(h))), ...rows],
...
`);
  log(
    `${import_chalk122.default.gray("Amount due:")} ${import_chalk122.default.bold(formatCurrency(data.grandTotals.billedCost))}`
  );
}
...
function outputBreakdown({
  ...
  log(
    `${periodLabel} Usage for ${import_chalk123.default.bold(data.contextName)} ${elapsed(Date.now() - startTime)}`
  );
  log("");
  const periodSuffix = data.usingDefaults ? " (current month)" : "";
  log(
    `${import_chalk123.default.gray("Period:")} ${data.fromDisplay} to ${data.toDisplay}${periodSuffix}`
  );
  log(`${import_chalk123.default.gray("Charges processed:")} ${data.chargeCount}`);
  log(`${import_chalk123.default.gray("Pricing unit:")} ${data.pricingUnit}`);
  log("");
  const sortedServices = [...data.services.entries()].sort(
    (a, b) => b[1].billedCost - a[1].billedCost
  );
  ...
  for (const periodKey of sortedPeriods) {
    ...
    log(
      `${import_chalk123.default.bold(import_chalk123.default.cyan(periodKey))} (Total: ${formatQuantity(periodData.totalPricingQuantity, data.pricingUnit)}, ${formatCurrency(periodData.totalBilledCost)})`
    );
    ...
    const tablePrint = table(
      [headers.map((h) => import_chalk123.default.bold(import_chalk123.default.gray(h))), ...rows],
      { hsep: 4, align: ["l", "r", "r"] }
    ).replace(/^/gm, "  ");
    print(`${tablePrint}
`);
...
async function usage(client) {
  ...
  const start = Date.now();
  if (!asJson) {
    spinner(`Fetching usage data for ${import_chalk124.default.bold(contextName)}`);
  }
...
`);
    ...
    output_manager_default.log(
      `${(0, import_pluralize13.default)("Webhook", webhooks.length, true)} found under ${import_chalk125.default.bold(
        contextName
      )} ${import_chalk125.default.gray(lsStamp())}`
    );
...
function formatWebhooksTable(webhooks) {
  ...
  const rows = webhooks.map((webhook) => {
    ...
    return [webhook.id, webhook.url, eventsDisplay, import_chalk125.default.gray(age)];
  });
  return formatTable(
    ["ID", "URL", "Events", "Age"],
    ["l", "l", "l", "l"],
...
`);
    ...
    output_manager_default.log(
      `Webhook ${webhookId} found under ${import_chalk126.default.bold(contextName)} ${import_chalk126.default.gray(
        getStamp()
      )}`
    );
    output_manager_default.print("\n");
    output_manager_default.print(import_chalk126.default.bold("  General\n\n"));
    output_manager_default.print(`    ${import_chalk126.default.cyan("ID")}			${webhook.id}
`);
    output_manager_default.print(`    ${import_chalk126.default.cyan("URL")}			${webhook.url}
`);
    output_manager_default.print(
      `    ${import_chalk126.default.cyan("Created At")}		${formatDate(webhook.createdAt)}
`
    );
    output_manager_default.print(
      `    ${import_chalk126.default.cyan("Updated At")}		${formatDate(webhook.updatedAt)}
`
    );
    output_manager_default.print("\n");
    output_manager_default.print(import_chalk126.default.bold("  Events\n\n"));
...
`);
    ...
    if (webhook.projectIds && webhook.projectIds.length > 0) {
      output_manager_default.print("\n");
      output_manager_default.print(import_chalk126.default.bold("  Projects\n\n"));
...
async function create2(client, argv) {
  ...
  let parsedArgs;
  const flagsSpecification = getFlagsSpecification(createSubcommand2.options);
  try {
    parsedArgs = parseArguments(argv, flagsSpecification);
  } catch (error) {
    printError(error);
  ...
  try {
    ...
    output_manager_default.print("\n");
    output_manager_default.print(import_chalk127.default.bold("  Webhook Details\n\n"));
    output_manager_default.print(`    ${import_chalk127.default.cyan("ID".padEnd(10))}${webhook.id}
`);
    output_manager_default.print(`    ${import_chalk127.default.cyan("URL".padEnd(10))}${webhook.url}
`);
    output_manager_default.print(
      `    ${import_chalk127.default.cyan("Events".padEnd(10))}${webhook.events.join(", ")}
`
    );
    if (webhook.projectIds && webhook.projectIds.length > 0) {
      output_manager_default.print(
        `    ${import_chalk127.default.cyan("Projects".padEnd(10))}${webhook.projectIds.join(", ")}
`
      );
    }
    output_manager_default.print("\n");
    output_manager_default.warn(
      `Save this secret - it will not be shown again: ${import_chalk127.default.bold(webhook.secret)}`
    );
    output_manager_default.print("\n");
    return 0;
  } catch (err) {
...
async function rm7(client, argv) {
  ...
  if (args.length !== 1) {
    output_manager_default.error(
      `Invalid number of arguments. Usage: ${import_chalk128.default.cyan(
  ...
  switch (subcommand) {
    case "create":
      if (needHelp) {
        ...
        return printHelp(createSubcommand2);
      }
      telemetry2.trackCliSubcommandCreate(subcommandOriginal);
      return create2(client, args);
    case "get":
      if (needHelp) {
        telemetry2.trackCliFlagHelp("webhooks", subcommandOriginal);
